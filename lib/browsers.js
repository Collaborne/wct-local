...
function expand(names, done) {
  ...
  module.exports.detect(function(error, installedByName) {
    ...
    console.log(`Expected $${JSON.stringify(names)}, found ${JSON.stringify(installed)}, missing ${JSON.stringify(missing)}`);
    if (missing.length > 0) {
      return done(
...
function detect(done) {
  launchpad.local(function(error, launcher) {
    ...
    launcher.browsers(function(error, browsers) {
      ...
      var results = {};
      console.log(`Got browsers from launcher:`);
      console.log(browsers);
      for (var i = 0, browser; browser = browsers[i]; i++) {
        if (!LAUNCHPAD_TO_SELENIUM[browser.name]) {
          console.log(`skipping ${browser.name} because it's not in LAUNCHPAD_TO_SELENIUM`);
          continue;
        }
